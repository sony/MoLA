...
from mola.models.operator.mola_module import SANConv1d

class MoLAVAESAN(nn.Module):
    ...
    def __init__(self,
        ...
        #add for SAN
        self.discriminator = SANDiscriminator(nfeats, output_emb_width=512, down_t=2, stride_t=2, width=512, depth=3, dilation_growth_rate=1, activation='leakyrelu', norm=None)
    ...
    def decode(self, z: Tensor, lengths: List[int]):
        ...
        if self.vae.arch == "all_encoder":
            ...
            if self.vae.pe_type == "actor":
                ...
                queries = self.vae.query_pos_decoder(queries)
                # mem_pos = self.vae.mem_pos_decoder(z)
                output = self.vae.decoder(
                    tgt=queries,
                    memory=z,
    ...
    def adversarial(
        ...
        adv_loss = self.discriminator(x, target, flg_train=flg_train)
        return adv_loss
...
class SANDiscriminator(nn.Module):
    ...
    def feature(self, x):
        ...
        logits_fun, logits_dir = self.last_layer(prediction, flg_train=True)
        return logits_fun
    ...
    def forward(self, x, target, flg_train=False):
        ...
        if target == "gen":
            ...
            minval = torch.min(logits_fun - 1, torch.zeros(logits_fun.shape[0], logits_fun.shape[1], logits_fun.shape[2]).to(logits_fun.device))
